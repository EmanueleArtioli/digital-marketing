...
df_6_dist_clicked <- df_6_camp_event_clean_final %>%
  group_by(CLICKED) %>%
  summarize(TOT_EVENTS = n_distinct(ID_EVENT_S)) %>%
  mutate(PERCENT = TOT_EVENTS/sum(TOT_EVENTS)) %>%
  arrange(desc(PERCENT)) %>%
  as.data.frame()

df_6_dist_clicked %>% head()

df_6_dist_clickbytypcamp <- df_6_camp_event_clean_final %>%
  group_by(TYP_CAMP, CLICKED) %>%
  summarize(COUNT = n()) %>%
  as.data.frame()

df_6_dist_clickbytypcamp %>% head()

# - FAILED/FAILED by TYP_CAP

df_6_dist_failed <- df_6_camp_event_clean_final %>%
  group_by(FAILED) %>%
  summarize(TOT_EVENTS = n_distinct(ID_EVENT_S)) %>%
  mutate(PERCENT = TOT_EVENTS/sum(TOT_EVENTS)) %>%
  arrange(desc(PERCENT)) %>%
...
df_6_dist_failed %>% head()

df_6_dist_failedbytypcamp <- df_6_camp_event_clean_final %>%
  group_by(TYP_CAMP, FAILED) %>%
  summarize(COUNT = n()) %>%
  as.data.frame()

df_6_dist_failedbytypcamp %>% head()

# - NUM_OPENs

df_6_dist_numopen <- df_6_camp_event_clean_final %>%
  group_by(NUM_OPENs) %>%
  summarize(TOT_EVENTS = n_distinct(ID_EVENT_S)) %>%
  mutate(PERCENT = TOT_EVENTS/sum(TOT_EVENTS)) %>%
  arrange(desc(PERCENT)) %>%
...
df_6_dist_numopen %>% head()

# - NUM_CLICKs

df_6_dist_numclick <- df_6_camp_event_clean_final %>%
  group_by(NUM_CLICKs) %>%
  summarize(TOT_EVENTS = n_distinct(ID_EVENT_S)) %>%
  mutate(PERCENT = TOT_EVENTS/sum(TOT_EVENTS)) %>%
  arrange(desc(PERCENT)) %>%
